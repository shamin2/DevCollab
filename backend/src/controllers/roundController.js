...
export async function createRound(req, res) {
    ...
    try {
        ...
        const roomResult = await client.query(`SELECT id, code, mode
       FROM rooms
       WHERE code = $1`, [code.toUpperCase()]);
        ...
        if (room.mode === "decision") {
            if (!Array.isArray(options) ||
                ...
                return res.status(400).json({
                    message: "Decision rounds require at least 2 options",
                });
            }
        }
        await client.query("BEGIN");
        const roundResult = await client.query(`INSERT INTO rounds (room_id, prompt)
       VALUES ($1, $2)
       RETURNING *`, [room.id, prompt.trim()]);
        ...
        if (room.mode === "decision") {
            for (const option of options) {
                if (typeof option !== "string" ||
                    ...
                    return res.status(400).json({
                        message: "All options must be valid text",
                    });
                }
                const optionResult = await client.query(`INSERT INTO options (round_id, label)
             VALUES ($1, $2)
             RETURNING *`, [round.id, option.trim()]);
...
export async function revealRound(req, res) {
    try {
       ...
       WHERE rounds.id = $1`, [roundId]);
        ...
        const hostResult = await pool.query(`SELECT id
       FROM members
       WHERE id = $1
         AND room_id = $2
         AND is_host = TRUE`, [memberId, round.room_id]);
        if (hostResult.rows.length === 0) {
            return res.status(403).json({
                message: "Only the host can reveal the round",
            });
        }
        // Reveal the round
        const result = await pool.query(`UPDATE rounds
       SET status = 'revealed'
       WHERE id = $1
       RETURNING *`, [roundId]);
        // Tell everyone that the round was revealed
        const io = req.app.get("io");
        io.to(round.code).emit("round-revealed", {
            roundId,
        });
...
export async function getRoundResults(req, res) {
    try {
        const roundId = Number(req.params.roundId);
        if (!Number.isInteger(roundId)) {
            return res.status(400).json({
       ...
       WHERE rounds.id = $1`, [roundId]);
        ...
        if (round.status !== "revealed") {
            return res.status(400).json({
                message: "Round has not been revealed yet",
            });
        }
        // -------------------------
        ...
        if (round.mode === "planning_poker") {
            const votesResult = await pool.query(`SELECT members.id AS member_id,
                  members.name,
                  votes.poker_value
           FROM votes
           JOIN members
           ...
           ORDER BY members.name`, [roundId]);
            const votes = votesResult.rows;
            const numericVotes = votes
                .map((vote) => Number(vote.poker_value))
        ...
        if (round.mode === "decision") {
            const optionsResult = await pool.query(`SELECT options.id,
                  options.label,
